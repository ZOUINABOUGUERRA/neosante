...
export const generateDossierPDF = functions.https.onCall(async (data, context) => {
  ...
  doc.text(`Document généré par NéoSanté - ${new Date().toLocaleDateString('fr-FR')}`, 50, footerY);
  
  // QR Code placeholder
  doc.rect(470, footerY - 10, 80, 80).stroke();
  doc.fontSize(8).text('QR Code', 500, footerY + 30, { align: 'center' });
  
  // ✅ إصلاح: استخدام دالة بدون معاملات
...
// ✅ دوال المساعدة
function addSection(doc: any, title: string): void {
  doc.moveDown();
...
function addField(doc: any, label: string, value: string): void {
  doc.font('Helvetica-Bold').text(`${label}: `, { continued: true });
  doc.font('Helvetica').text(value);
}

function addCheckField(doc: any, label: string, value: boolean): void {
