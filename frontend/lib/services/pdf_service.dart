...
class PdfService {
  ...
  static Future<File> generateAndSave(String dossierId, String dossierType) async {
    try {
  ...
  static Future<pw.Document> generateDossierPDF(String dossierId, String dossierType) async {
    try {
  ...
  static Future<void> generateAndShare(String dossierId, String dossierType) async {
    try {
  ...
  static Future<void> generateAndPrint(String dossierId, String dossierType) async {
    try {
      final pdf = await generateDossierPDF(dossierId, dossierType);
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.Row(
            children: [
              _buildInfoColumn('Date naissance', _formatDate(data['birthDateTime'])),
              _buildInfoColumn('Âge gestationnel', '${data['gestationalAge'] ?? '?'} SA'),
            ],
          ),
        ],
      ),
        ...
        children: [
          ...
          pw.Row(
            children: [
              ...
              _buildInfoColumn('Température', '${data['bodyTemperature'] ?? '?'} °C'),
              _buildInfoColumn('Glycémie', '${data['bloodGlucose'] ?? '?'} mg/dL'),
            ],
          ),
          pw.SizedBox(height: 8),
          pw.Row(
        ...
        pw.Row(
          ...
          children: [
            pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Text('Document généré par NéoSanté', style: pw.TextStyle(fontSize: 8, color: PdfColors.grey)),
                pw.Text('Date: ${DateTime.now().toLocal().toString().split(' ')[0]}', style: pw.TextStyle(fontSize: 8, color: PdfColors.grey)),
              ],
            ),
            pw.Container(
              width: 80,
